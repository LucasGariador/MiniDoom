    ...
    if(currentState == MAINMENU) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_MOUSEBUTTONDOWN) {
                    ...
                    // Si haces clic en el bot�n Play
        ...
        return; // Salir temprano si estamos en el men�
	}
...
void Game::update() {
    ...
    for (Sprite* s : sprites) {
        // Si ya lo agarramos o est� muerto, ignorar
        if (s->isDead) continue;

        // 1. CHEQUEO DE DISTANCIA (Colisi�n circular)
        float dx = s->x - playerX;
        float dy = s->y - playerY;
        // Distancia al cuadrado (m�s r�pido que sqrt)
        float distSq = dx * dx + dy * dy;
    ...
    // ------------------------------------------------------------
    // Usamos una l�gica unificada para borrar balas y enemigos muertos
...
void Game::renderSprites() {
    ...
    // 2. Ordenar toda la lista usando la posición del jugador (playerX, playerY)
    ...
    std::sort(spritesToRender.begin(), spritesToRender.end(), [this](Sprite* a, Sprite* b) {
        // Distancia al cuadrado para optimizar (nos ahorramos la raíz cuadrada)
        double distA = ((a->x - playerX) * (a->x - playerX) + (a->y - playerY) * (a->y - playerY));
        double distB = ((b->x - playerX) * (b->x - playerX) + (b->y - playerY) * (b->y - playerY));
        
        // Retornamos true si 'a' está MÁS LEJOS que 'b' para que se dibuje primero
        return distA > distB; 
    });
...
void Game::checkShooting(Sprite* enemy, float playerX, float playerY, float playerAngle, const std::vector<float>& zBuffer, int SCREEN_WIDTH) {
    ...
    // Si el �ngulo es muy peque�o lo tenemos en la mira
    // Tambi�n verificamos que est� cerca (rango del arma)
    if (fabs(angleToEnemy) < 0.15f && distToEnemy < 10.0f) {

        // 4. Muro en el medio (Oclusi�n)
        // Miramos el ZBuffer en el centro de la pantalla (Screen Width / 2)
        // Si la pared est� m�s cerca que el enemigo, fallamos el tiro.
        float wallDist = zBuffer[SCREEN_WIDTH / 2];

        if (distToEnemy < wallDist) {
			enemy->takeDamage(50); // 50 de da�o, TODO: variable arma
        }
    }
}
    ...
    const std::vector<float>& zBuffer) // opcional si quers pintar alcance de rayos
{
    ...
    for (int x = 0; x < rays; x += MINI_RAYS_STEP) {
        // �ngulo del rayo x (ajusta a tu c�lculo real)
        float rayAngle = (playerAngle - fov * 0.5f) + (float)x / (float)rays * fov;

        // Punto final aproximado usando una distancia recortada (si no ten�s zBuffer a mano)
        ...
        // Si ten�s zBuffer[x] con la distancia corregida, usalo:
    ...
    // Direcci�n del jugador (flecha corta)
...
void Game::clean() {
    ...
    // Apagar librer�as en orden inverso
    IMG_Quit();
    TTF_Quit();
    SDL_Quit();
